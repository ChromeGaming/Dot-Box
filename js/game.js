class Game {
	...
	constructor(rows, columns, playersCount) {
		...
		this.updatePlayerScoreUI = this.updatePlayerScoreUI.bind(this);
		this.onBoxFill = this.onBoxFill.bind(this);
		this.onPlayerSwitch = this.onPlayerSwitch.bind(this);
	...
	onPlayerWin() {
		...
		const player = this.players.reduce((prev, current) =>
			prev.filledBoxes > current.filledBoxes ? prev : current
		);
	...
	onBoxFill() {
		...
		requestAnimationFrame(this.updatePlayerScoreUI);
	}
